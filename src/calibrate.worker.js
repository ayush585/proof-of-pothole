...
self.onmessage = async (event) => {
  ...
  if (type === "classify") {
    try {
      ...
      const areaComponent = Math.log10(1 + area_px / 500);
      const edgeComponent = Math.log10(1 + edgeCount);
      const score = Math.round(0.6 * meanDark + 0.4 * areaComponent * 100 + 0.2 * edgeComponent * 60);
